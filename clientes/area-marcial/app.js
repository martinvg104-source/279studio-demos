/* Área Marcial — app-shell de reservas (web pública + alumno + admin). Vanilla JS.
   Router por hash + vistas + wizard de reserva (7 pasos) + panel de administración.
...
(function () {
  ...
  /* ---------------- router ---------------- */
  var TITLES = {
    inicio: "Inicio", reservar: "Reservar", "mis-reservas": "Mis reservas", actividades: "Disciplinas",
    horarios: "Horarios", avisos: "Avisos", perfil: "Perfil",
    resumen: "Resumen", "admin-reservas": "Reservas", "admin-calendario": "Calendario",
    "admin-alumnos": "Alumnos", "admin-grupos": "Disciplinas y grupos", "admin-solicitudes": "Solicitudes de clase de prueba",
    "admin-avisos": "Avisos y comunicaciones", "admin-automatizaciones": "Automatizaciones", "admin-config": "Configuración"
  };
  var VIEWS = {
    ...
    "admin-alumnos": renderAdminAlumnos, "admin-grupos": renderAdminGrupos, "admin-solicitudes": renderAdminSolicitudes,
    "admin-avisos": renderAdminAvisos, "admin-automatizaciones": renderAdminAutomatizaciones, "admin-config": renderAdminConfig
  };
  ...
  function setupNavForRole(role) {
    ...
    sidebarRoleLabel.textContent = role === "admin" ? "Acceso academia · admin" : "Acceso alumno";
  }
  ...
  function enterApp(role, name) {
    CURRENT_USER = { role: role, name: name || (role === "admin" ? "Área Marcial" : "Alumno") };
    if (role === "alumno") {
      PROFILE.name = CURRENT_USER.name;
      PROFILE.initials = initialsOf(CURRENT_USER.name);
    }
    setupNavForRole(role);
    $("#userMenuInitials").textContent = initialsOf(CURRENT_USER.name);
  ...
  function showLoginMode(mode) {
    ...
    $("#loginHint").hidden = isRegister;
    $("#loginTitle").textContent = "Área Marcial";
    $("#loginSubtitle").textContent = isRegister ? "Crea tu cuenta de alumno" : "Accede a tu espacio de Área Marcial";
    $("#loginSwitchText").textContent = isRegister ? "¿Ya tienes cuenta?" : "¿No tienes cuenta?";
    $("#btnLoginSwitch").textContent = isRegister ? "Iniciar sesión" : "Crear una cuenta";
    loginError.hidden = true;
  }
  ...
  loginForm.addEventListener("submit", function (ev) {
    ...
    if (account && account.password === pass) {
      loginError.hidden = true;
      enterApp(account.role, account.name);
    } else {
  ...
  if (registerForm) {
    var rfDisciplina = $("#rf-disciplina", registerForm);
    if (rfDisciplina) rfDisciplina.innerHTML = ACTIVITIES.map(function (a) { return '<option value="' + a.id + '">' + a.name + "</option>"; }).join("");
    registerForm.addEventListener("submit", function (ev) {
      ev.preventDefault();
      var nombre = $("#rf-nombre", registerForm).value.trim();
      var email = $("#rf-email", registerForm).value.trim().toLowerCase();
      var pass = $("#rf-pass", registerForm).value;
      var edad = $("#rf-edad", registerForm).value;
      var disciplinaId = $("#rf-disciplina", registerForm).value;
      var username = email.split("@")[0] || email;
      LOGIN_USERS[username] = { password: pass, role: "alumno", name: nombre };
      var newId = genId("AL");
      STUDENTS.push({ id: newId, nombre: nombre, edad: edad, tutor: "", telefono: "", email: email, activityId: disciplinaId, group: (GROUPS[disciplinaId] || ["Grupo único"])[0], nivel: "Sin grado asignado (dato simulado)", estado: "activo", ultimaAsistencia: "", seguimiento: false, notas: ["Alta por registro público (demo)."] });
      toast("Cuenta creada en la demo. Accede con " + username + " y la contraseña que has elegido.");
      registerForm.reset();
      showLoginMode("login");
      $("#li-email", loginForm).value = username;
    });
  }
  ...
  $("#btnLoginSwitch").addEventListener("click", function () { showLoginMode(loginMode === "login" ? "register" : "login"); });
  $("#btnHeaderLogin").addEventListener("click", function () { showLogin("login"); });
  $("#btnHeaderRegister").addEventListener("click", function () { showLogin("register"); });
  $("#btnHeaderArea").addEventListener("click", function () { showLogin("login"); });
  $("#mobileNavRegister").addEventListener("click", function (ev) { ev.preventDefault(); showLogin("register"); });
  $("#mobileNavArea").addEventListener("click", function (ev) { ev.preventDefault(); showLogin("login"); });
  $("#btnHeroAccount").addEventListener("click", function () { showLogin("login"); });
  $("#btnBackToSite").addEventListener("click", function (ev) { ev.preventDefault(); showPublicSite(); });
  $("#sidebarBackToSite").addEventListener("click", function () { logout(); });

  var siteMenuToggle = $("#siteMenuToggle");
  var siteNav = $("#siteNav");
  if (siteMenuToggle) {
    ...
    $all("#siteNav a").forEach(function (a) { a.addEventListener("click", function () { siteNav.classList.remove("is-open"); }); });
  }

  var userMenuToggle = $("#userMenuToggle");
  var userMenuDropdown = $("#userMenuDropdown");
  if (userMenuToggle) {
    userMenuToggle.addEventListener("click", function (ev) {
      ev.stopPropagation();
      var open = userMenuDropdown.hidden;
      userMenuDropdown.hidden = !open;
      userMenuToggle.setAttribute("aria-expanded", String(open));
    });
    document.addEventListener("click", function (ev) {
      if (!userMenuDropdown.hidden && !userMenuDropdown.contains(ev.target) && ev.target !== userMenuToggle) userMenuDropdown.hidden = true;
    });
    $all("[data-usermenu]", userMenuDropdown).forEach(function (a) { a.addEventListener("click", function () { userMenuDropdown.hidden = true; }); });
    $("#userMenuLogout").addEventListener("click", function () { userMenuDropdown.hidden = true; logout(); });
  }
  ...
  function renderInicio(root) {
    ...
    root.appendChild(el(
      ...
      '<div class="quick-row">' +
        ...
        '<a class="quick-item card" href="#/actividades">' + icon("grid") + "<span>Disciplinas</span></a>" +
  ...
  function stepGrupo() {
    ...
    return '<div class="notice-banner">Grupos de demostración. La academia debe validar la organización real de los grupos.</div>' +
  ...
  function stepHorario() {
    ...
    return '<div class="notice-banner">Ejemplo de disponibilidad · horario orientativo · confirmar con la academia.</div>' +
  ...
  function stepDatos() {
    return '<form id="wizForm" novalidate><div class="field"><label for="f-nombre">Nombre y apellidos del alumno</label><input id="f-nombre" required value="' + booking.nombre + '"></div>' +
      ...
      '<label class="field-check"><input type="checkbox" id="f-contacto" required' + (booking.contacto ? " checked" : "") + "> Acepto que la academia se ponga en contacto conmigo para confirmar esta solicitud.</label>" +
      "</form>" + wizardNavHtml();
  }
  ...
  function stepConfirmacion() {
    ...
    return '<div class="confirm-screen"><div class="confirm-icon">' + icon("check") + "</div>" +
      ...
      "</div>" +
      "<p>Solicitud de clase de prueba registrada en la demo. En el sistema real, la academia recibiría el aviso automáticamente y la familia recibiría la confirmación con las instrucciones necesarias.</p>" +
  ...
  function renderActividades(root) {
    ...
    root.appendChild(el(
      '<div><div class="toolbar">' +
        ...
        '<select class="filter-select" id="actTipo"><option value="todas">Todas las categorías</option><option value="disciplina">Disciplinas</option><option value="programa">Programas</option></select>' +
        '<select class="filter-select" id="actNivel"><option value="todos">Todos los niveles</option><option value="Iniciación">Iniciación</option><option value="Iniciación e intermedio">Iniciación e intermedio</option><option value="Intermedio">Intermedio</option><option value="Avanzado">Avanzado</option><option value="Todos los niveles">Todos los niveles</option></select>' +
  ...
  function renderHorarios(root) {
    ...
    root.innerHTML = "";
    root.appendChild(el(
      '<div><div class="notice-banner">Horarios de demostración. La programación real debe ser validada por la academia.</div>' +
  ...
  function simulatedAction(label) {
    toast("Acción simulada: " + label + ". En la versión real se conectaría con el CRM y los canales de comunicación de la academia.");
  }
  ...
  function renderResumen(root) {
    ...
    var solicitudesNuevas = TRIAL_REQUESTS.filter(function (s) { return s.estado === "nueva"; }).length;
  ...
  function activityFormFields(a) {
    ...
    return '<div class="field"><label for="actf-nombre">Nombre</label><input id="actf-nombre" required value="' + (a.name || "") + '"></div>' +
      ...
      '<div class="field"><label for="actf-tipo">Categoría</label><select id="actf-tipo"><option value="disciplina">Disciplina</option><option value="programa">Programa</option></select></div>';
  }
  ...
  /* ---------------- Admin: Solicitudes de clase de prueba ---------------- */
  var SOLICITUD_ESTADOS = ["nueva", "contactada", "pendiente-confirmacion", "confirmada", "realizada", "convertida", "cerrada"];
  var SOLICITUD_LABEL = {
    "nueva": "Nueva", "contactada": "Contactada", "pendiente-confirmacion": "Pendiente de confirmación",
    "confirmada": "Confirmada", "realizada": "Realizada", "convertida": "Convertida en alumno", "cerrada": "Cerrada"
  };
  var adminSolicitudesFilter = "todas";

  function renderAdminSolicitudes(root) {
    root.innerHTML = "";
    root.appendChild(el(adminBadge()));
    root.appendChild(el('<p class="section-lead">Solicitudes de clase de prueba recibidas desde la web pública. Contacta manualmente y actualiza el estado según avance.</p>'));
    var filterHtml = '<div class="toolbar"><div class="view-toggle" style="flex-wrap:wrap;">' +
      '<button type="button" data-sf="todas">Todas</button>' +
      SOLICITUD_ESTADOS.map(function (e) { return '<button type="button" data-sf="' + e + '">' + SOLICITUD_LABEL[e] + "</button>"; }).join("") +
      "</div></div><div id=\"solicitudesBody\"></div>";
    root.appendChild(el('<div>' + filterHtml + '</div>'));
    $all("[data-sf]", root).forEach(function (btn) {
      btn.classList.toggle("is-active", btn.getAttribute("data-sf") === adminSolicitudesFilter);
      btn.addEventListener("click", function () { adminSolicitudesFilter = btn.getAttribute("data-sf"); renderAdminSolicitudes(root); });
    });

    var body = $("#solicitudesBody", root);
    var items = TRIAL_REQUESTS.filter(function (s) { return adminSolicitudesFilter === "todas" || s.estado === adminSolicitudesFilter; });
    body.innerHTML = items.length ? '<div class="admin-table">' + items.map(function (s) {
      var a = activityById(s.disciplina);
      return '<div class="admin-row">' +
        '<div class="admin-row-main"><strong>' + s.nombre + " " + s.apellidos + " (" + s.edad + " años)</strong>" +
        "<span>" + (a ? a.name : s.disciplina) + " · " + s.nivel + " · " + s.horario + " · " + s.telefono + " · " + s.email + "</span></div>" +
        '<span class="status-pill status-' + (s.estado === "cerrada" ? "cancelada" : (s.estado === "confirmada" || s.estado === "convertida" || s.estado === "realizada" ? "confirmada" : "pendiente")) + '">' + SOLICITUD_LABEL[s.estado] + "</span>" +
        '<div class="admin-row-actions"><select class="filter-select" data-solicitud-estado="' + s.id + '">' +
        SOLICITUD_ESTADOS.map(function (e) { return '<option value="' + e + '"' + (e === s.estado ? " selected" : "") + ">" + SOLICITUD_LABEL[e] + "</option>"; }).join("") +
        "</select>" +
        '<button type="button" class="btn btn-sm btn-ghost" data-solicitud-contact="' + s.id + '">Contactar</button></div></div>';
    }).join("") + "</div>" : '<div class="empty-state">' + icon("bolt") + "<p>No hay solicitudes en esta vista.</p></div>";

    $all("[data-solicitud-estado]", body).forEach(function (sel) {
      sel.addEventListener("change", function () {
        var s = TRIAL_REQUESTS.filter(function (x) { return x.id === sel.getAttribute("data-solicitud-estado"); })[0];
        s.estado = sel.value;
        toast("Solicitud actualizada a \"" + SOLICITUD_LABEL[s.estado] + "\" en la demo.");
        renderAdminSolicitudes(root);
      });
    });
    $all("[data-solicitud-contact]", body).forEach(function (btn) {
      btn.addEventListener("click", function () { simulatedAction("Contactar solicitante"); });
    });
  }
  ...
  function renderAdminConfig(root) {
    root.innerHTML = "";
    root.appendChild(el(adminBadge()));
    root.appendChild(el('<p class="section-lead">Datos de contacto público de la academia. Editar aquí cuando Área Marcial confirme datos reales.</p>'));
    root.appendChild(el(
      '<form id="configForm" class="card card-pad">' +
      '<div class="field"><label for="cfg-name">Nombre de la academia</label><input id="cfg-name" value="' + CLUB.name + '"></div>' +
      '<div class="field"><label for="cfg-address">Dirección</label><input id="cfg-address" value="' + CLUB.address + '"></div>' +
      '<div class="field"><label for="cfg-location">Ubicación dentro del centro</label><input id="cfg-location" value="' + CLUB.locationNote + '"></div>' +
      '<div class="field"><label for="cfg-phone">Teléfono</label><input id="cfg-phone" value="' + CLUB.phone + '"></div>' +
      '<div class="field"><label for="cfg-instagram">Instagram</label><input id="cfg-instagram" value="' + CLUB.instagram + '"></div>' +
      '<div class="field"><label for="cfg-schedule">Horario público orientativo</label><input id="cfg-schedule" value="' + CLUB.scheduleNote + '"></div>' +
    ...
    $("#configForm", root).addEventListener("submit", function (ev) {
      ev.preventDefault();
      CLUB.name = $("#cfg-name", root).value; CLUB.address = $("#cfg-address", root).value; CLUB.locationNote = $("#cfg-location", root).value;
      CLUB.phone = $("#cfg-phone", root).value; CLUB.instagram = $("#cfg-instagram", root).value;
      CLUB.scheduleNote = $("#cfg-schedule", root).value; CLUB.notice = $("#cfg-notice", root).value;
      toast("Datos de la academia actualizados en la demo.");
    });
  }

  /* ---------------- Web pública ---------------- */
  function initPublicSite() {
    $("#benefitsGrid").innerHTML = BENEFITS.map(function (b) {
      return '<div class="card benefit-card">' + icon(b.icon) + "<p>" + b.title + "</p></div>";
    }).join("");

    $("#disciplineGrid").innerHTML = DISCIPLINES.map(function (d) {
      return '<div class="card discipline-card">' +
        "<h3>" + d.name + "</h3>" +
        (d.confirmed ? "" : '<span class="chip chip-consult">Programa disponible bajo consulta</span>') +
        '<p class="discipline-meta">' + d.forWhom + "</p>" +
        '<p class="discipline-benefits">' + d.what + "</p>" +
        '<div class="discipline-tags"><span class="chip">' + d.level + "</span><span class=\"chip\">" + d.benefits + "</span></div>" +
        '<div class="discipline-actions"><a href="#clase-prueba" class="btn btn-sm btn-primary" data-trial-disc="' + d.id + '">Reservar clase de prueba</a></div>' +
        "</div>";
    }).join("");
    $all("[data-trial-disc]", $("#disciplineGrid")).forEach(function (a) {
      a.addEventListener("click", function () { var sel = $("#tf-disciplina"); if (sel) sel.value = a.getAttribute("data-trial-disc"); });
    });

    $("#audienceGrid").innerHTML = AUDIENCES.map(function (a) {
      return '<div class="card audience-card"><h3>' + a.title + "</h3><p>" + a.text + "</p></div>";
    }).join("");

    $("#philosophyGrid").innerHTML = PHILOSOPHY_VALUES.map(function (p) {
      return '<div class="philosophy-item"><h4>' + p.title + "</h4><p>" + p.text + "</p></div>";
    }).join("");

    var discSel = $("#pubSchedDisc");
    discSel.innerHTML += ACTIVITIES.map(function (a) { return '<option value="' + a.id + '">' + a.name + "</option>"; }).join("");
    var diaSel = $("#pubSchedDia");
    diaSel.innerHTML += DOW.map(function (d) { return '<option value="' + d + '">' + d + "</option>"; }).join("");

    function drawPublicSchedule() {
      var discFilter = discSel.value, diaFilter = diaSel.value;
      var rows = [];
      SCHEDULE.forEach(function (s) {
        if (discFilter !== "todas" && s.activityId !== discFilter) return;
        var a = activityById(s.activityId);
        s.days.forEach(function (d) {
          if (diaFilter !== "todos" && d !== diaFilter) return;
          rows.push("<tr><td>" + a.name + "</td><td>" + d + "</td><td>" + s.time + "</td><td>" + a.availability + "</td><td>" + (a.availability.indexOf("consulta") !== -1 ? "Bajo consulta" : "Programada") + "</td></tr>");
        });
      });
      $("#pubSchedBody").innerHTML = rows.length ? rows.join("") : '<tr><td colspan="5" style="text-align:center;color:var(--muted);">Sin resultados para este filtro.</td></tr>';
    }
    discSel.addEventListener("change", drawPublicSchedule);
    diaSel.addEventListener("change", drawPublicSchedule);
    drawPublicSchedule();

    var tfDisc = $("#tf-disciplina");
    tfDisc.innerHTML = ACTIVITIES.map(function (a) { return '<option value="' + a.id + '">' + a.name + "</option>"; }).join("");

    $("#trialForm").addEventListener("submit", function (ev) {
      ev.preventDefault();
      var wrap = $("#trialFormWrap");
      TRIAL_REQUESTS.unshift({
        id: genId("SOL"), nombre: $("#tf-nombre").value, apellidos: $("#tf-apellidos").value, email: $("#tf-email").value,
        telefono: $("#tf-telefono").value, edad: Number($("#tf-edad").value), disciplina: tfDisc.value, nivel: $("#tf-nivel").value,
        horario: $("#tf-horario").value, observaciones: $("#tf-obs").value, consentimiento: true, estado: "nueva", fecha: isoDate(todayDate())
      });
      wrap.innerHTML = '<div class="trial-confirm"><div class="trial-confirm-icon">' + icon("check") + "</div>" +
        "<h3>Solicitud recibida</h3><p style=\"color:var(--muted);margin-top:8px;\">Gracias, hemos registrado tu solicitud de clase de prueba en la demo. En la versión real, el equipo de Área Marcial te contactaría para confirmar día y hora. Esto no confirma una plaza automáticamente.</p></div>";
      toast("Solicitud de clase de prueba registrada (simulado).");
    });

    $("#contactForm").addEventListener("submit", function (ev) {
      ev.preventDefault();
      toast("Mensaje enviado en la demo. En una versión real, la academia lo recibiría por email.");
      $("#contactForm").reset();
    });

    $("#contactAddress").textContent = CLUB.address + " · " + CLUB.locationNote;
    $("#contactPhone").textContent = CLUB.phone;
    $("#contactSchedule").textContent = CLUB.scheduleNote;
    $("#contactInstagram").href = CLUB.instagram;
    $("#contactCall").href = "tel:" + CLUB.phoneHref;
    $("#contactWhatsapp").href = CLUB.whatsappHref;
    $("#footerAddress").textContent = CLUB.address;
