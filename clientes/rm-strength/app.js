/* RM Strength — app-shell de reservas. Vanilla JS, sin dependencias.
...
(function () {
  ...
  var TITLES = {
    ...
    "admin-alumnos": "Clientes", "admin-grupos": "Servicios y niveles", "admin-avisos": "Avisos y comunicaciones",
  ...
  function setupNavForRole(role) {
    ...
    sidebarRoleLabel.textContent = role === "admin" ? "Acceso centro · admin" : "Acceso cliente";
  }
  ...
  function renderInicio(root) {
    ...
    var upcomingHtml = upcoming
      ? '<div class="next-card"><p class="next-card-label">Tu próxima sesión</p><p class="next-card-title">' + activityById(upcoming.activityId).name + "</p>" +
        '<p class="next-card-meta">' + formatHuman(upcoming.date) + " · " + upcoming.time + " · " + upcoming.student + "</p></div>"
      : '<div class="next-card"><p class="next-card-label">Aún no tienes sesiones próximas</p><p class="next-card-title">Reserva tu primera sesión de valoración</p></div>';
    ...
    root.appendChild(el(
      ...
      upcomingHtml +
      '<button type="button" class="cta-big" id="ctaReservar">' + icon("plus") + "Reservar una sesión</button>" +
      '<div class="quick-row">' +
        '<a class="quick-item card" href="#/mis-reservas">' + icon("list") + "<span>Mis sesiones</span></a>" +
        '<a class="quick-item card" href="#/actividades">' + icon("grid") + "<span>Servicios</span></a>" +
        '<a class="quick-item card" href="#/avisos">' + icon("bell") + "<span>Avisos</span></a>" +
      "</div>" +
      featuredHtml +
      '<div class="stat-grid" style="margin-bottom:var(--s-5);">' +
        '<div class="card stat-card"><p class="stat-num">' + PROFILE.students.length + '</p><p class="stat-label">Objetivo activo</p></div>' +
        '<div class="card stat-card"><p class="stat-num">' + completedCount + '</p><p class="stat-label">Sesiones completadas (resumen)</p></div>' +
      "</div>" +
      ...
      '<div class="card trial-card">' +
        '<div class="trial-card-copy"><strong>Sesión de valoración</strong><span>Tu progreso empieza con una primera sesión.</span></div>' +
        '<button type="button" class="btn btn-outline" id="ctaTrial">Reservar sesión de valoración</button>' +
      "</div>" +
      "</div>"
    ));
  ...
  function stepGrupo() {
    ...
    return '<div class="notice-banner">Niveles de demostración. El centro debe validar la organización real de los niveles.</div>' +
  ...
  function stepHorario() {
    ...
    return '<div class="notice-banner">Ejemplo de disponibilidad · horario orientativo · confirmar con el centro.</div>' +
  ...
  function stepDatos() {
    return '<form id="wizForm" novalidate><div class="field"><label for="f-nombre">Nombre y apellidos del cliente</label><input id="f-nombre" required value="' + booking.nombre + '"></div>' +
      '<div class="field"><label for="f-edad">Edad del cliente</label><input id="f-edad" type="number" min="14" max="99" required value="' + booking.edad + '"></div>' +
      '<div class="field"><label for="f-tutor">Restricciones médicas (si las hay)</label><input id="f-tutor" value="' + booking.tutor + '"></div>' +
      ...
      '<div class="field"><label for="f-nivel">Objetivo principal</label><select id="f-nivel"><option value="">Sin especificar</option><option value="perdida-grasa">Pérdida de grasa</option><option value="ganancia-muscular">Ganancia muscular</option><option value="rehabilitacion">Rehabilitación</option><option value="rendimiento">Rendimiento</option><option value="salud">Salud general</option></select></div>' +
      '<div class="field"><label for="f-obs">Observaciones</label><textarea id="f-obs" rows="3">' + booking.observaciones + "</textarea></div>" +
      '<label class="field-check"><input type="checkbox" id="f-contacto" required' + (booking.contacto ? " checked" : "") + "> Acepto que el centro se ponga en contacto conmigo para confirmar esta solicitud.</label>" +
      "</form>" + wizardNavHtml();
  }
  ...
  function stepResumen() {
    ...
    return '<dl class="summary-list">' +
      ...
      "<dt>Cliente</dt><dd>" + booking.nombre + "</dd>" +
  ...
  function stepConfirmacion() {
    ...
    return '<div class="confirm-screen"><div class="confirm-icon">' + icon("check") + "</div>" +
      ...
      "</div>" +
      "<p>Solicitud de sesión de valoración registrada en la demo. En el sistema real, el centro recibiría el aviso automáticamente y el cliente recibiría la confirmación con las instrucciones necesarias.</p>" +
  ...
  function renderMisReservas(root) {
    ...
    root.appendChild(el('<button type="button" class="btn btn-primary btn-block" id="btnRebook" style="margin-top:20px;">Reservar otra sesión</button>'));
  ...
  function renderActividades(root) {
    ...
    root.appendChild(el(
      '<div><div class="toolbar">' +
        ...
        '<select class="filter-select" id="actTipo"><option value="todas">Todas las categorías</option><option value="individual">Individual</option><option value="grupo">Grupo reducido</option><option value="equipo">Equipo</option></select>' +
        '<select class="filter-select" id="actNivel"><option value="todos">Todos los niveles</option><option value="Iniciación">Iniciación</option><option value="Todos los niveles">Todos los niveles</option><option value="Rendimiento">Rendimiento</option></select>' +
  ...
  function renderHorarios(root) {
    ...
    root.innerHTML = "";
    root.appendChild(el(
      '<div><div class="notice-banner">Horarios de demostración. La programación real debe ser validada por el centro.</div>' +
  ...
  function weekViewHtml() {
    ...
    for (var i = 0; i < 7; i++) {
      ...
      days += '<div class="card week-day"><p class="week-day-name">' + dayName + " " + d.getDate() + "</p>" +
        ...
        }).join("") : '<p class="week-empty">Sin sesiones</p>') + "</div>";
    }
  ...
  function scheduleDetailHtml(detail) {
    ...
    return '<div class="card detail-card"><p class="detail-title">' + a.name + "</p>" +
      ...
      '<button type="button" class="btn btn-primary btn-block" id="detailReservar">Reservar esta sesión</button></div>';
  }
  ...
  function renderPerfil(root) {
    ...
    root.appendChild(el('<p class="section-title">Objetivo actual</p>'));
    root.appendChild(el('<div class="card card-pad" id="studentsCard">' + PROFILE.students.map(function (s) {
      return '<div class="student-row"><div><strong>' + s.name + "</strong><br><span style=\"color:var(--muted);font-size:0.84rem;\">" + s.activity + "</span></div>" +
        '<span class="belt-chip" style="background:var(--action);color:#fff;">' + s.belt + "</span></div>";
    }).join("") + "</div>"));
  ...
  /* ---------------- Avisos (alumno) ---------------- */
  var CATEGORY_LABEL = { general: "General", horarios: "Horarios", pagos: "Pagos", actividades: "Actividades" };
  ...
  function simulatedAction(label) {
    toast("Acción simulada: " + label + ". En la versión real se conectaría con el CRM y los canales de comunicación del centro.");
  }
  ...
  function renderResumen(root) {
    ...
    root.appendChild(el('<div class="stat-grid">' +
      ...
      '<div class="card stat-card"><p class="stat-num">' + pruebasPendientes + '</p><p class="stat-label">Sesiones de valoración pendientes</p></div>' +
      '<div class="card stat-card"><p class="stat-num">' + alumnosActivos + '</p><p class="stat-label">Clientes activos</p></div>' +
      ...
      '<div class="card stat-card"><p class="stat-num">' + seguimiento + '</p><p class="stat-label">Clientes pendientes de seguimiento</p></div>' +
  ...
  function renderAdminReservas(root) {
    ...
    function rowActions(r) {
      ...
      btns.push('<button type="button" class="btn btn-sm btn-ghost" data-contact="' + r.id + '">Contactar cliente</button>');
      return btns.join("");
    }
    function draw() {
      ...
      body.innerHTML = items.length ? '<div class="admin-table">' + items.map(function (r) {
        ...
        return '<div class="admin-row">' +
          ...
          '<span>' + formatHuman(r.date) + " · " + r.time + " · " + r.student + (r.trial ? " · <em>sesión de valoración</em>" : "") + "</span></div>" +
      ...
      $all("[data-contact]", body).forEach(function (btn) { btn.addEventListener("click", function () { simulatedAction("Contactar cliente"); }); });
    ...
    root.appendChild(el(
      '<div>' +
      '<div class="toolbar">' +
      '<input type="search" class="field-input search-input" placeholder="Buscar cliente..." id="resSearch">' +
  ...
  function openCreateClassModal(redraw) {
    openModal("Crear sesión", '<form id="classForm">' + classFormFields() + '<button type="submit" class="btn btn-primary btn-block">Crear sesión</button></form>');
    ...
    $("#classForm").addEventListener("submit", function (ev) {
      ...
      closeModal();
      toast("Sesión creada en la demo. En una versión real, aparecería en el calendario de clientes y se enviarían los avisos correspondientes.");
      redraw();
    });
  }

  function openEditClassModal(c, redraw) {
    openModal("Editar sesión", '<form id="classForm">' + classFormFields(c) + '<button type="submit" class="btn btn-primary btn-block">Guardar cambios</button></form>');
    wireClassFormActivity();
    $("#classForm").addEventListener("submit", function (ev) {
      ev.preventDefault();
      ...
      closeModal();
      toast("Sesión actualizada en la demo.");
      redraw();
    });
  }
  ...
  function openAttendanceModal(c) {
    ...
    var a = activityById(c.activityId);
    if (!roster.length) {
      openModal("Registrar asistencia", '<p class="section-lead">' + a.name + " · " + c.group + '</p><div class="empty-state">' + icon("user") + "<p>No hay clientes de demostración en este grupo.</p></div>");
      return;
    }
    var state = {};
  ...
  function renderAdminCalendario(root) {
    ...
    root.appendChild(el(
      ...
      '<button type="button" class="btn btn-primary btn-sm" id="btnCreateClass">+ Crear sesión</button>' +
    ...
    if (adminCalUI.mode === "dia") {
      ...
      body.appendChild(el(classes.length ? '<div class="admin-table">' + classes.map(classRowHtml).join("") + "</div>" : '<div class="empty-state">' + icon("calendar") + "<p>Sin sesiones este día.</p></div>"));
      ...
      body.appendChild(el('<div class="notice-banner">Vista mensual de ejemplo: los puntos marcan días de la semana con sesiones programadas.</div>'));
      body.appendChild(el('<div class="day-select">' + DOW.map(function (d) {
      ...
      body.appendChild(el(mClasses.length ? '<div class="admin-table">' + mClasses.map(classRowHtml).join("") + "</div>" : '<div class="empty-state">' + icon("calendar") + "<p>Sin sesiones este día.</p></div>"));
      $all("[data-day]", body).forEach(function (btn) { btn.addEventListener("click", function () { adminCalUI.day = btn.getAttribute("data-day"); renderAdminCalendario(root); }); });
    }
    ...
    $all("[data-del-class]", body).forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        toast("Sesión eliminada en la demo.");
  ...
  function studentFichaHtml(s) {
    ...
    return '<div class="ficha-block"><p class="info-row"><dt>Edad</dt><dd>' + s.edad + " años</dd></p>" +
      ...
      '<p class="info-row"><dt>Objetivo / estado</dt><dd>' + s.nivel + "</dd></p>" +
  ...
  function openStudentFicha(s, redraw) {
    openModal(s.nombre, studentFichaHtml(s) +
      ...
      '<button type="button" class="btn btn-sm btn-outline" id="fichaContact">Contactar cliente</button>' +
      '<button type="button" class="btn btn-sm ' + (s.estado === "activo" ? "btn-danger-outline" : "btn-outline") + '" id="fichaToggleEstado">' + (s.estado === "activo" ? "Marcar como inactivo" : "Reactivar cliente") + "</button></div>"
    );
    ...
    $("#fichaContact", modalBody).addEventListener("click", function () { simulatedAction("Contactar cliente"); });
    $("#fichaToggleEstado", modalBody).addEventListener("click", function () {
      s.estado = s.estado === "activo" ? "inactivo" : "activo";
      closeModal();
      toast(s.estado === "activo" ? "Cliente reactivado en la demo." : "Cliente marcado como inactivo en la demo.");
      redraw();
    });
  ...
  function openStudentEditModal(s, redraw) {
    ...
    openModal(isNew ? "Añadir cliente" : "Editar ficha",
      '<form id="studentForm">' +
      '<div class="field"><label for="sf-nombre">Nombre y apellidos</label><input id="sf-nombre" required value="' + s.nombre + '"></div>' +
      '<div class="field"><label for="sf-edad">Edad</label><input id="sf-edad" type="number" min="14" required value="' + s.edad + '"></div>' +
      '<div class="field"><label for="sf-tutor">Restricciones médicas (si las hay)</label><input id="sf-tutor" value="' + s.tutor + '"></div>' +
      ...
      '<div class="field"><label for="sf-activity">Servicio</label><select id="sf-activity">' + ACTIVITIES.map(function (a) { return '<option value="' + a.id + '"' + (a.id === s.activityId ? " selected" : "") + ">" + a.name + "</option>"; }).join("") + "</select></div>" +
      '<div class="field"><label for="sf-group">Nivel</label><select id="sf-group">' + groupOptionsHtml(s.activityId, s.group) + "</select></div>" +
      '<div class="field"><label for="sf-nivel">Objetivo / estado (dato simulado)</label><input id="sf-nivel" value="' + s.nivel + '"></div>' +
      '<button type="submit" class="btn btn-primary btn-block">' + (isNew ? "Añadir cliente" : "Guardar cambios") + "</button></form>"
    );
    ...
    $("#studentForm", modalBody).addEventListener("submit", function (ev) {
      ...
      closeModal();
      toast(isNew ? "Cliente añadido en la demo." : "Ficha actualizada en la demo.");
      redraw();
    });
  }
  ...
  function openAccessFormModal(redraw) {
    openModal("Crear acceso",
      ...
      '<div class="field"><label for="af-student">Cliente asociado</label><select id="af-student">' + STUDENTS.map(function (s) { return '<option value="' + s.id + '">' + s.nombre + "</option>"; }).join("") + "</select></div>" +
      '<div class="field"><label for="af-tipo">Tipo de cuenta</label><select id="af-tipo"><option value="cliente">Cliente</option><option value="familiar">Familiar</option></select></div>' +
  ...
  function renderAdminAlumnos(root) {
    ...
    root.innerHTML = "";
    root.appendChild(el(adminBadge()));
    root.appendChild(el('<div class="view-toggle" style="margin-bottom:20px;"><button type="button" id="tabAlumnos">Clientes</button><button type="button" id="tabAccesos">Gestionar accesos</button></div>'));
    ...
    if (ui.tab === "alumnos") {
      body.appendChild(el(
        '<div>' +
        '<div class="toolbar">' +
        '<input type="search" class="field-input search-input" placeholder="Buscar cliente..." id="alSearch">' +
        ...
        '<button type="button" class="btn btn-primary btn-sm" id="btnAddStudent">Añadir cliente</button>' +
      ...
      function drawList() {
        ...
        }).join("") + "</div>" : '<div class="empty-state">' + icon("user") + "<p>No hay clientes que coincidan.</p></div>";
      ...
      function drawAccess() {
        ...
        list.innerHTML = ACCESSES.length ? '<div class="admin-table">' + ACCESSES.map(function (a) {
          ...
          return '<div class="admin-row"><div class="admin-row-main"><strong>' + a.nombre + "</strong><span>" + a.email + " · " + (a.tipo === "familiar" ? "Familiar" : "Cliente") + " · Asociado a " + (s ? s.nombre : "—") + "</span></div>" +
  ...
  function activityFormFields(a) {
    ...
    return '<div class="field"><label for="actf-nombre">Nombre</label><input id="actf-nombre" required value="' + (a.name || "") + '"></div>' +
      ...
      '<div class="field"><label for="actf-tipo">Categoría</label><select id="actf-tipo"><option value="individual">Individual</option><option value="grupo">Grupo reducido</option><option value="equipo">Equipo</option></select></div>';
  }
  ...
  function renderAdminAvisos(root) {
    ...
    $("#btnCreateNotice", root).addEventListener("click", function () {
      ...
      $("#noticeForm", modalBody).addEventListener("submit", function (ev) {
        ...
        toast(status === "publicado"
          ? "Aviso publicado en la demo. En una versión real, los clientes recibirían una notificación según sus preferencias."
    ...
    $all("[data-publish-notice]", body).forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        toast("Aviso publicado en la demo. En una versión real, los clientes recibirían una notificación según sus preferencias.");
        refreshNavBadges();
        renderAdminAvisos(root);
      });
    });
  ...
  function renderAdminConfig(root) {
    root.innerHTML = "";
    root.appendChild(el(adminBadge()));
    root.appendChild(el('<p class="section-lead">Datos de contacto público del centro. Editar aquí cuando RM Strength confirme datos reales.</p>'));
    root.appendChild(el(
      '<form id="configForm" class="card card-pad">' +
      '<div class="field"><label for="cfg-name">Nombre del centro</label><input id="cfg-name" value="' + CLUB.name + '"></div>' +
    ...
    $("#configForm", root).addEventListener("submit", function (ev) {
      ...
      CLUB.phone = $("#cfg-phone", root).value; CLUB.email = $("#cfg-email", root).value;
      CLUB.instagram = $("#cfg-instagram", root).value; CLUB.notice = $("#cfg-notice", root).value;
      toast("Datos del centro actualizados en la demo.");
    });
  }
